...
const NutritionistView: React.FC<NutritionistViewProps> = ({ 
  ...
  const handleDownloadPlanPDF = (plan: MealPlan) => {
    ...
    doc.setFontSize(20);
    doc.text(plan.planName, 105, 20, { align: 'center' });
    ...
    doc.text(plan.description, 20, 40, { maxWidth: 170 });

    // Add section for daily meals
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    doc.text('Daily Meals:', 20, 55);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(12);
    doc.text(`Breakfast: ${plan.dailyMeals.breakfast.map(item => item.name).join(', ')}`, 20, 65, { maxWidth: 170 });
    doc.text(`Lunch: ${plan.dailyMeals.lunch.map(item => item.name).join(', ')}`, 20, 75, { maxWidth: 170 });
    doc.text(`Dinner: ${plan.dailyMeals.dinner.map(item => item.name).join(', ')}`, 20, 85, { maxWidth: 170 });
    doc.text(`Snacks: ${plan.dailyMeals.snacks.map(item => item.name).join(', ')}`, 20, 95, { maxWidth: 170 });

    // Add weekly variety tips if available
    if (plan.weeklyVarietyNotes) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(14);
        doc.text('Weekly Variety Tips:', 20, 110);

        doc.setFont('helvetica', 'normal');
        doc.setFontSize(12);
        doc.text(plan.weeklyVarietyNotes, 20, 115, { maxWidth: 170 });
    }
    ...
    if (plan.benefits) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(14);
        doc.text('Benefits:', 20, 130);

        doc.setFont('helvetica', 'normal');
        doc.setFontSize(12);
        doc.text(plan.benefits, 20, 135, { maxWidth: 170 });
    }

    // Add nuances/tips if available
    if (plan.nuances) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(14);
        doc.text('Nuances/Tips:', 20, 150);

        doc.setFont('helvetica', 'normal');
        doc.setFontSize(12);
        doc.text(plan.nuances, 20, 155, { maxWidth: 170 });
    }

    // Add footer
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(10);
    doc.text('Generated by Nutritionist Dashboard', 105, 290, { align: 'center' });

    // Save the PDF
    doc.save(`${plan.planName.replace(/\s+/g, '_')}_MealPlan.pdf`);
  };
              ...
              <div key={index} className="p-4 bg-white rounded-lg shadow-md border border-gray-200 transition-all duration-300 hover:shadow-lg">
                ...
                <div className="mt-4 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                    ...
                    <button
                        ...
                        className="flex-1 px-3 py-2 text-xs bg-red-500 text-white font-semibold rounded-md shadow hover:bg-red-600 disabled:opacity-50 flex items-center justify-center"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-4 h-4 mr-1">
