  ...
  return (
    <header className="mb-6 sm:mb-8 text-center relative">
      <div className="inline-flex items-center bg-[var(--color-header-bg)] p-3 sm:p-4 rounded-lg shadow-xl relative">
        {userEmail && (
          <span className="text-sm text-[var(--color-text-secondary)] hidden sm:inline mr-3 sm:mr-4 truncate max-w-xs" title={userEmail}>{userEmail}</span>
        )}
        {onLogout && (
          <button 
            onClick={onLogout}
            title="Logout"
            className="p-2 rounded-md hover:bg-[var(--color-background-hover)] transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 mr-3 sm:mr-4"
          >
